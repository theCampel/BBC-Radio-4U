...
def main():
    ...
    if dummy_mode:
        ...
        # Normal mode: actually process RSS feeds to get fresh articles
        news_processor = NewsProcessor()

        # Choose how many articles per source. We distribute the total max (e.g. 10)
        source_selector = SourceSelector()
        selected_sources = []
        for key, (name, url) in SourceSelector.AVAILABLE_SOURCES.items():
            selected_sources.append((name, url))
        distributed = source_selector._distribute_articles(selected_sources)
        
        # Fetch articles
        news_processor.process_selected_sources(distributed)
    ...
    while True:
        ...
        if (len(play_queue) - current_index) < 3:
            expand_queue(play_queue, dummy_mode, spotify_handler, news_processor,
        ...
        if item["type"] == "conversation":
            ...
            # If we ever hit a placeholder un-generated (shouldn't normally happen now),
            ...
            # While playing, check if the next item is a conversation_placeholder
